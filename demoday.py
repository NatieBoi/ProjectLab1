...
# Role: attacker targets enemy goal, defender carries to own goal
IS_ATTACKER = bool(_sw_role.value())
ROLE_STR    = "ATTACKER" if IS_ATTACKER else "DEFENDER"
...
def turn_to_face(tx, ty, speed=35):
    ...
    while True:
        ...
        if abs(error) < 12.0:
            break
        # Softer ramp, hard slow-down inside 20deg, floor at 15
        spd = max(15, min(speed, int(abs(error) * 0.6)))
        if abs(error) < 20:
            spd = 15
        now_us  = time.ticks_us()
        dt      = time.ticks_diff(now_us, last_us) * 1e-6
...
def fire_solenoid():
    print("[KICK] Waiting 2s before nudge")
    time.sleep_ms(2000)
    print("[KICK] Nudging forward")
    motorL_forward(percent_to_duty(30))
    motorR_forward(percent_to_duty(30))
    time.sleep_ms(150)
    brake()
    print("[KICK] Waiting 2s after nudge")
    time.sleep_ms(2000)
...
NEAR_GOAL_SHOOT_MM     = 10
FAR_GOAL_REPOSITION_MM = 30
...
def select_target_goal(ball_color):
    """Return (target_x, target_y, label) using role, ball colour, and field geometry.

    Rules:
      - RED ball   → always attack enemy goal regardless of role
      - NOT RED    → attacker scores at enemy goal, defender returns to own goal
    Own/enemy sides are determined from starting position:
      B1 and B2 both start at low X (left side), so enemy goal is at high X.
    """
    if ball_color == "RED" or IS_ATTACKER:
        return ENEMY_GOAL_X_MM, GOAL_Y_CENTER, "ENEMY"
    else:
        return OUR_GOAL_X_MM, GOAL_Y_CENTER, "OWN"
...
# =========================================================
# SECTION 25: OLED BALL DISPLAY  (defined early — used by phases)
# =========================================================
def update_oled_ball(target_x=None, target_y=None, target_label=""):
    ...
    if target_x is not None:
        ...
        # No active target — show distance to both goals for situational awareness
...
# =========================================================
# SECTION 22: CARRY TO OWN GOAL
# =========================================================
def phase_carry_to_own_goal(target_x, target_y, target_label):
    global _phase_last_us, _pivot_mode
    print("=== PHASE: CARRY — target {} ({:.0f},{:.0f}) ===".format(
        target_label, target_x, target_y))
    CARRY_TIMEOUT_MS = 25000
    t_start = time.ticks_ms()
    _pivot_mode = False
    turn(180, speed=40)
    time.sleep_ms(200)
    ...
    while time.ticks_diff(time.ticks_ms(), t_start) < CARRY_TIMEOUT_MS:
        _phase_tick()
        read_all_sonars()
        ...
        # Arrived when X is within 50mm of destination goal line
        if IS_ATTACKER:
            arrived = pos_x >= DEST_X - 50
        else:
            arrived = pos_x <= DEST_X + 50

        if arrived:
            print("[CARRY] At {} goal — depositing".format(target_label))
            fire_solenoid()
            _resync_loop_timer()
            return

        field_boundary_guard()
        forward(speed=55, duration_ms=500)
        _phase_last_us = time.ticks_us()
        time.sleep_ms(20)
...
# =========================================================
# Resync timer AFTER the sweep so first dt isn't garbage
_resync_loop_timer()
_last_loop_ms = time.ticks_ms()

DRIVE_TIMEOUT_MS = 8000   # max time allowed chasing a ball in the main loop

print("=== STARTING MAIN LOOP ===")

try:
    phase_initial_sweep()
    print("=== INITIAL SWEEP COMPLETE — ENTERING MAIN LOOP ===")

    # Resync again so first loop dt is clean
    _resync_loop_timer()
    _last_loop_ms = time.ticks_ms()
    ...
    while True:
        ...
        if dt > 0.5:          # clamp runaway dt (e.g. after long phase)
            dt = 0.5
        ...
        # ── PRIORITY 3: COLOR — timer-driven at 33Hz ────────────
        if flag_color:
            flag_color = False
            poll_color()
        ...
        if cached_direction is None:
            ...
            ball_color = None
            base       = 18
            drive_t_start = time.ticks_ms()   # ← timeout anchor
            ...
            if ball_color == "RED":
                ...
                phase_carry_to_own_goal(gx, gy, glabel)
